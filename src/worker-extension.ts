...
export async function activate(context: ExtensionContext) {
  const ExtStatusBarItem = window.createStatusBarItem(
    ...
    errorHandler: {
      error(
        ...
        window.showErrorMessage(
          `${error.toString()} | ${message?.toString()} | ${count}`
        );
  ...
  const disposable = createWorkerLanguageClient(context, clientOptions);
  ...
  disposable.onReady().then(() => {
    ...
    commands.executeCommand(ELS_COMMANDS.SET_CONFIG, {
      local: {
